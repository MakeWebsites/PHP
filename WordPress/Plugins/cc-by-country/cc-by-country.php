...
include_once "includes/cctitle.php";
  $sci = '<a name="ccg"></a>';
  if ($cclink === 'true') { //Including the link
	...
	$sci .= '<div align="right"><a href="'.$clink.'" target="_blank">
	<img class="align-top" src="'.$infop.'">'.__("Read more", "cc-by-country").'</a></div>';
	 }
  $sci .= new cctitle($c2, $c3);
    ...
    // Form    
        $scf = '<form method="POST" action="'.get_permalink().'#ccg">';
		$scf .= '<button type="submit" style="float:right">'.__("Submit", "cc-by-country").'</button>';
		$scf .= '<div style="overflow: hidden; padding-right: .5em;">';
        $scf .= '<select id="country2" name="country2"><option selected value="US">'.__('Select another country....', 'cc-by-country').'</option>';
